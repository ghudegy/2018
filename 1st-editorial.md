...
9. 그냥 스타트링크 홈페이지로 가면 정답 6을 알 수 있다. 시간이 지나서 스타트링크에 새로운 서비스가 추가될 수도 있는데, 모든 답은 2018년 4월 1일을 기준으로 한다고 했으므로 답은 여전히 6이다. 이 때는 Wayback Machine을 타고 들어가면 알 수 있다.
...
### 🐱. Glen
문제가 길지만 사실은 간단하고, 풀이도 길이에 비해 어렵지 않다. 출제자는 다음과 같은 방법을 사용했다.

기본적인 틀은 첫 행을 왼쪽에서 오른쪽으로 채우고, 다음 행을 오른쪽에서 왼쪽으로 채우고, ... 반복하는 것이다. 왼쪽에서 오른쪽으로 채울 때 다음 칸을 채우려면, 일단 다음 칸으로 간다. 이 때 칸의 상태가 우리가 원하는 상태와 같으면 끝이고, 아니면 오른쪽으로 한 번 간 다음 왼쪽으로 한 번 간다. 각 칸은 최대 세 번만에 처리되므로 제약조건 안에 문제를 해결할 수 있다.
...
우선 기본적으로 알 수 있는 사실은 **주사위가 클 필요가 없다**는 것, **뱀을 많이 배치해야 한다**는 것이다. M=2로 잡아 보자. 그러면 두 칸마다 뱀을 설치해야 될 것처럼 보이지만, 세 칸마다 설치하는 것이 오히려 이득이다. 두 칸마다 설치하면 1/2의 확률을 50번 버텨내야 하고, 세 칸마다 설치하면 66번 버텨내야 하기 때문이다. 그러면 뱀을 총 33마리 설치할 수 있다. 3, 6, 9, ..., 99에 뱀을 설치하고, 각 뱀의 도착점을 1, 2, 4, 5, 7, 8, 10, 11, ..., 49로 두면 통과된다. 이 때의 기대값은 문제에서 언급한 79036724938709836284이다.

참가자의 답 중 가장 큰 기대값은 (todo)이었다.
