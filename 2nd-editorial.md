...
문제에 주어진 것을 그대로 구현해도 시간복잡도 상으로 잘 돌아간다. 단, Python은 굉장히 느린 언어이므로 Pypy와 Pike로 제출해야 한다. (현재 추가 시간 제한이 없어서 Pike로 풀 수 없으나, 제거될 예정이다.)
...
아래와 같은 데이터가 있는 것이다. (놀랍게도 [실제로 그런 문제가 있었다.](https://www.acmicpc.net/board/view/31764))
```
5
10 11 12 13
```
...
참고로 문제 이름은 "김파이"인데 주인공 이름은 "김우리"이다. 김우리가 가장 좋아하는 언어가 Python이 아니라 Pike인 이유는 BOJ에서 Python이 문제풀이에 매우 부적절한 언어로 잘 알려져 있기 때문이다.
...
전체적인 풀이는 [16993번 연속합과 쿼리](https://https://www.acmicpc.net/problem/16993) 문제와 비슷합니다.
...
문제를 다시 잘 읽어보자. 1 이상 5 이하의 숫자를 아무렇게나 N개 출력하면 된다.
